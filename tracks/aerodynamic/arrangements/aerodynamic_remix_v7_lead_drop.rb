# Lead-drop remix derived from aerodynamic_remix_v6_polished_arc.rb.
#
...
# V6 remains intact except for a focused eight-bar buildup whose final silence
# makes the defining isolated lead arrive with greater tension and release.
...
# Two deterministic drum-build stages used before the defining lead entrance.
define :lead_build_drums_bar do |stage|
  if stage == :quarter
    events = [
      [0.0, [[:bd_haus, 0.24]]],
      [0.5, [[:drum_cymbal_closed, 0.08]]],
      [1.0, [[:sn_dub, 0.14]]],
      [1.5, [[:drum_cymbal_closed, 0.08]]],
      [2.0, [[:bd_haus, 0.24]]],
      [2.5, [[:drum_cymbal_closed, 0.08]]],
      [3.0, [[:sn_dub, 0.16]]],
      [3.5, [[:drum_cymbal_closed, 0.08]]]
    ]
  else
    snare_onsets = [0.0, 1.0, 2.0, 2.5, 3.0, 3.25, 3.5, 3.75]
    snare_levels = [0.12, 0.13, 0.14, 0.15, 0.16, 0.17, 0.18, 0.20]
    events = [
      [0.0, [[:bd_haus, 0.20]]],
      [2.0, [[:bd_haus, 0.20]]]
    ]
    snare_onsets.each_with_index do |onset, index|
      events.push [onset, [[:sn_dub, snare_levels[index]]]]
    end
    events = events.sort_by { |event| event[0] }
  end
...
# A 12-beat low-pass rise. The envelope ends 1.8 beats before the helper does.
define :lead_build_riser do
  with_fx :lpf, cutoff: 70, cutoff_slide: 4 do |build_filter_fx|
    use_synth :noise
    synth :noise, attack: 3.0, sustain: 6.8,
      release: 0.4, amp: 0.028
    control build_filter_fx, cutoff: 86
    sleep 4
    control build_filter_fx, cutoff_slide: 6, cutoff: 98
    sleep 8
  end
end

# GP4 opening-bass events truncated before the late F-sharp pickup.
define :opening_bass_cut_bar do
  events = [
    [0.0, 1.0, :b2, 1.0],
    [1.5, 0.5, :b2, 1.0]
  ]
...
# Three beats of familiar funk material followed by one full silent beat.
define :lead_entry_final_bar do
  in_thread do
    events = [
      [0.5, :d4, 1.12, 104, 0.22],
      [1.0, :b3, 0.84, 96, 0.11],
  ...
  in_thread do
    events = [
      [0.0, :bd_haus, 0.22, 0.12],
      [1.0, :sn_dub, 0.16, 0.10],
      [1.5, :drum_tom_lo_hard, 0.15, 0.10],
      [2.0, :sn_dub, 0.18, 0.10],
      [2.5, :perc_snap, 0.16, 0.08]
    ]
    cursor = 0.0
    events.each do |event|
      onset = event[0].to_f
      sleep onset - cursor if onset > cursor
      sample event[1], amp: event[2], release: event[3]
      cursor = onset
    end
    sleep 4.0 - cursor
  end
  ...
  in_thread do
    with_fx :lpf, cutoff: 70, cutoff_slide: 2.5 do |final_filter_fx|
      use_synth :noise
      synth :noise, attack: 1.8, sustain: 0.55,
        release: 0.2, amp: 0.025
      control final_filter_fx, cutoff: 96
      sleep 4
    end
  end
...
define :transition_riser do
  with_fx :rhpf, cutoff: 58, cutoff_slide: 4, res: 0.35 do |filter_fx|
    use_synth :noise
    synth :noise, attack: 2.5, sustain: 7.2, release: 0.4, amp: 0.032

    use_synth :hollow
    play :d2, attack: 2.5, sustain: 7.2, release: 0.4,
      cutoff: 68, amp: 0.032

    control filter_fx, cutoff: 78
    sleep 4
    control filter_fx, cutoff: 98
    sleep 8
  end
end
...
# 0:16-1:12: extended groove followed by an eight-bar lead buildup.
...
opening_groove_bars.times do |bar|
  ...
  if bar >= 20
    if bar < 24
      ...
      if bar < 23
        build_bass_amps = [0.265, 0.255, 0.245]
        build_bass_cutoffs = [64, 62, 60]
        in_thread do
          opening_bass_bar source_measure - 9,
            build_bass_amps[bar - 20], build_bass_cutoffs[bar - 20]
        end
      end
      ...
      kick_amp = [0.25, 0.25, 0.24, 0.22][bar - 20]
      in_thread { club_kick_bar kick_amp, false }

      if bar == 20
        in_thread do
          remix_pad_bar chord_notes, 0.075, 86, true, -0.05
        end
      elsif bar == 21
        in_thread { remix_stabs_bar chord_notes, 0.11, 90, 0.12 }
      elsif bar == 22
        in_thread do
          remix_pad_bar chord_notes, 0.06, 86, true, 0.05
        end
      else
        in_thread { funk_micro_chop_bar 0.12 }
      end
    elsif bar == 24
      in_thread { lead_build_riser }
      in_thread { lead_build_drums_bar :quarter }
    elsif bar == 25
      in_thread { lead_build_drums_bar :accelerate }
    elsif bar == 26
      in_thread { opening_bass_cut_bar }
      in_thread { club_kick_bar 0.22, true }
    else
      in_thread { lead_entry_final_bar }
    end
  elsif full_groove_bar
    in_thread do
      funk_melody_bar [12, 16].include?(source_measure),
        :front, filter_stage, bar % 4
    end
