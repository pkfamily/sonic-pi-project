# Sample-backed mix refinement derived from aerodynamic_remix_v10_mix_polish.rb.
#
# V10's 416-beat structure, cue map, gain staging, and protected silences are
# unchanged. The recurring GP4 funk melody now uses the approved FreePats clean
# electric guitar multisample, and the supplemental dpulse chord stabs are
# removed to leave more space around the core groove.
...
define :tapping_bar do |notes|
  use_synth :zawa
  use_synth_defaults attack: 0.04, sustain: 0.12,
    release: 0.10, cutoff: 100, amp: 0.48
  4.times do
    notes.each do |note|
      play note
      sleep 0.25
...
define :transition_stutter_bar do |lead_notes|
  ...
  in_thread do
    ...
    3.times do
      lead_notes.each do |note|
        pan_value = note_index.even? ? -0.09 : 0.09
        play note, attack: 0.01, sustain: 0, release: 0.14,
          cutoff: 98, amp: 0.054, pan: pan_value
        note_index += 1
        sleep 0.25
      end
    end
...
define :remix_lead_double do |notes, amp_value, cutoff_value|
  ...
  4.times do
    notes.each do |note|
      pan_value = note_index.even? ? -0.09 : 0.09
      play note, attack: 0.01, sustain: 0, release: 0.14,
        cutoff: [cutoff_value, 98].min,
        amp: amp_value * 0.72, pan: pan_value
      note_index += 1
      sleep 0.25
    end
  end
...
lead_stage_one_start_bar = opening_groove_start_bar +
  opening_groove_bars + lead_only_bars
