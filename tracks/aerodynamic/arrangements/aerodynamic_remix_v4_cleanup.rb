# Cleanup remix derived from aerodynamic_remix_v3_timing.rb.
#
...
# V3 timing is retained while percussion density, tonal stacking, transitions,
# and the final bell are refined.
...
define :post_hihat_bar do
  events = []
  16.times do |step|
    events.push [step * 0.25, [[:drum_cymbal_pedal, 0.20]]]
  end
  drum_bar events
end
...
opening_groove_bars.times do |bar|
  ...
  if bar >= 4 && bar < 8
    ...
    in_thread { club_kick_bar 0.21, false }
  elsif bar >= 16 && bar < 20
    in_thread { club_kick_bar 0.23, false }
  elsif bar >= 20 && bar < 24
    in_thread { club_kick_bar 0.26, false }
  elsif bar >= 24 && bar < 26
...
post_layered_bars.times do |layered_bar|
  ...
  if beat_only
    ...
    if layered_bar < 4
      ...
      # Strip long and late percussion before the controlled ending decay.
      in_thread { club_kick_bar 0.23, true }
    end

    pad_bar = [8, 10, 12, 14, 18].include?(layered_bar)
    stab_bar = [9, 11, 13, 15].include?(layered_bar)
    ...
    if [13, 15, 18].include?(layered_bar)
      in_thread do
        with_fx :pan, pan: 0.14 do
