# Polished-motion remix derived from aerodynamic_remix_v5_refined.rb.
#
...
# V5's dynamic arc is retained while deterministic groove, phrase filtering,
# and restrained stereo movement add a more natural production contour.
...
define :post_backbeat_bar do |ghost_amp|
  events = [
    [1.0, [[:sn_dub, 0.52]]],
  ...
  events.push [2.75, [[:sn_dub, ghost_amp]]] if ghost_amp > 0
...
opening_groove_bars.times do |bar|
  ...
  if bar >= 4 && bar < 8
    ...
    in_thread { beat_only_bar :club, 0.030, false }
  elsif bar == 26
    in_thread { filter_house_drums_bar :filtered_kick, false, 0.025 }
  elsif bar == 27
  ...
  if [9, 11, 13, 15, 17, 19, 21, 23].include?(bar)
    ...
    in_thread { remix_stabs_bar chord_notes, stab_amp, 90, 0.12 }
  end

  if [16, 18, 20, 22].include?(bar)
    pad_amp = bar >= 20 ? 0.075 : 0.06
    pad_pan = bar % 4 == 0 ? -0.05 : 0.05
    in_thread { remix_pad_bar chord_notes, pad_amp, 86, true, pad_pan }
  end
...
post_layered_bars.times do |layered_bar|
  ...
  if layered_bar == 16
    ...
    if layered_bar < 4
      ...
      in_thread { post_backbeat_bar 0 }
      in_thread { club_kick_bar 0.22, false }
    elsif layered_bar < 16
      in_thread { post_hihat_bar 0.21, 0.025, bar_index }
      in_thread { post_backbeat_bar 0.09 }
      in_thread { club_kick_bar 0.25, false }
    elsif layered_bar == 18
      in_thread { half_time_drums_bar }
    end
