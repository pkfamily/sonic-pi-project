# Professional mix polish derived from aerodynamic_remix_v9_structural_rework.rb.
#
# Hybrid choices retained from the working version:
# - four opening bells spread across the eight-bar introduction
# - one transition bell followed by a six-second interval
...
# V9's notes, timing, and structure remain intact beneath warmer gain staging,
# tighter drum envelopes, clearer foreground/background roles, and more headroom.

use_bpm 120

# Render profile used by the batch exporter. :full preserves V10 exactly;
# other profiles mute a layer while still consuming its scheduled beats.
set :render_profile, :full

define :render_group_enabled? do |group_name|
  profile = get(:render_profile) || :full
  profile == :full || profile == group_name
end
...
define :tonal_bar do |events, synth_name, base_amp, cutoff_value|
  tonal_group = [:tb303, :sine].include?(synth_name) ? :bass : :harmony
  return sleep 4 unless render_group_enabled? tonal_group
...
define :polished_drum_hit do |sample_name, amp_value, pan_value|
  return unless render_group_enabled? :drums
...
define :funk_melody_bar do |fill_bar, context, filter_stage, phrase_position|
  return sleep 4 unless render_group_enabled? :melody
  ...
  base_amp = context == :front ? 0.70 : 0.30
  pulse_level = context == :front ? 0.05 : 0.03
  ...
  with_fx :hpf, cutoff: 50, mix: 1.0 do
    with_fx :distortion, distort: 0.035, mix: 0.08 do
      events.each_with_index do |event, event_index|
        ...
        pan_value = event_index.even? ? -0.06 : 0.06
        ...
        # A quiet same-pitch pulse reinforces the pick without becoming a voice.
        use_synth :pulse
...
define :opening_bass_bar do |bar_index, amp_value, cutoff_value|
  return sleep 4 unless render_group_enabled? :bass
...
define :tapping_bar do |notes|
  return sleep 4 unless render_group_enabled? :lead
...
define :post_effect_chord_bar do |bar_index, pumped|
  return sleep 4 unless render_group_enabled? :harmony
...
define :intro_drone do
  return sleep 32 unless render_group_enabled? :fx
...
define :simple_lead_entry_bar do
  ...
  with_fx :hpf, cutoff: 50, mix: 1.0 do
    events.each_with_index do |event, event_index|
      onset = event[0].to_f
      pan_value = event_index.even? ? -0.04 : 0.04
      note_amp = 0.66 * event[2]
      sleep onset - cursor if onset > cursor
      ...
      play event[1], pulse_width: 0.18, attack: 0, sustain: 0,
        ...
        amp: note_amp * 0.035, pan: pan_value
      cursor = onset
    end
  end
...
define :ending_decay_bar do |beat_count|
  return sleep beat_count unless render_group_enabled? :fx
...
define :short_transition_bed do
  return sleep 8 unless render_group_enabled? :fx
