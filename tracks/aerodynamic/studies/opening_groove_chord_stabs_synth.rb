# Aerodynamic opening-groove chord-stab baseline study
#
# Twelve-bar A/B excerpt derived from opening_groove_clean_guitar_excerpt.rb.
# It preserves absolute opening bars 9-20 and the existing dpulse chord stabs.
...
define :remix_stabs_bar do |chord_notes, amp_value, cutoff_value,
  ...
  sleep 1.5
  play_chord chord_notes, attack: 0.005, sustain: 0, release: 0.18,
...
cue :section_chord_stabs_baseline
...
with_fx :level, amp: 0.90 do
  (8...20).each do |bar|
    ...
    if [9, 11, 13, 15, 17, 19].include?(bar)
      in_thread { remix_stabs_bar chord_notes, 0.20, 90, 0.12 }
    end
