# Progressive French-house remix derived from aerodynamic_funk_polished.rb.
#
...
define :funk_melody_bar do |fill_bar, context|
  events = [
  ...
  base_amp = context == :front ? 0.76 : 0.48
  cursor = 0.0
...
define :remix_opening_sub_bar do |bar_index, context|
  base_amp = context == :front ? 0.10 : 0.08
...
# Supplemental drums only; the original GP4 percussion remains untouched.
define :remix_drums_bar do |stage, omit_last_open|
  events = []
  closed_amp = stage == :climax ? 0.14 : 0.10
  snap_amp = stage == :climax ? 0.20 : 0.15
  kick_amp = stage == :climax ? 0.30 : 0.23
  open_amp = stage == :climax ? 0.16 : 0.12

  if [:hats, :light, :drive].include?(stage)
    [0.5, 1.5, 2.5, 3.5].each_with_index do |onset, index|
      pan_value = index.even? ? -0.22 : 0.22
      events.push [onset, :drum_cymbal_closed, closed_amp, pan_value]
    end
  elsif [:club, :climax].include?(stage)
    events.push [1.5, :drum_cymbal_closed, closed_amp, -0.18]
    events.push [3.5, :drum_cymbal_closed, closed_amp, 0.18]
    events.push [0.5, :drum_cymbal_open, open_amp, -0.12]
    events.push [2.5, :drum_cymbal_open, open_amp, 0.12] unless omit_last_open
  end

  if [:light, :drive, :club, :climax].include?(stage)
    events.push [1.0, :perc_snap, snap_amp, -0.08]
    events.push [3.0, :perc_snap, snap_amp, 0.08]
  end

  if [:drive, :club, :climax].include?(stage)
    events.push [0.0, :bd_haus, kick_amp, 0]
    events.push [1.5, :bd_haus, kick_amp * 0.78, 0]
    events.push [3.5, :bd_haus, kick_amp * 0.72, 0]
  end
...
define :remix_fill_bar do
  events = [
    [3.0, :elec_fuzz_tom, 0.22, -0.3],
    [3.25, :elec_fuzz_tom, 0.25, 0.3],
    [3.5, :elec_hi_snare, 0.25, -0.12],
    [3.75, :perc_snap2, 0.22, 0.12]
  ]
  cursor = 0.0
  events.each do |event|
    onset = event[0].to_f
...
opening_groove_bars.times do |bar|
  ...
  in_thread do
    funk_melody_bar [12, 16].include?(source_measure), :front
  end
  ...
  if bar >= 4 && bar < 8
    in_thread { remix_drums_bar :hats, false }
  elsif bar >= 8 && bar < 12
    in_thread { remix_drums_bar :hats, false }
    in_thread { remix_stabs_bar chord_notes, 0.08, 84 }
  elsif bar >= 12 && bar < 16
    in_thread { remix_drums_bar :drive, false }
  elsif bar >= 16
    in_thread { remix_drums_bar :club, bar == 21 }
    in_thread { remix_stabs_bar chord_notes, 0.09, 90 }
    in_thread { remix_pad_bar chord_notes, 0.06, 82, true }
  end

  in_thread { remix_fill_bar } if bar == 21
  sleep 4
end
...
lead_stage_two_bars.times do |bar|
  ...
  in_thread do
    funk_melody_bar [36, 40].include?(source_measure), :under_lead
  end
  in_thread { opening_bass_bar source_measure - 29 }
  in_thread { remix_opening_sub_bar source_measure - 29, :under_lead }
  in_thread { percussion_one_bar source_measure.odd? }
  ...
  in_thread { remix_drums_bar drum_stage, final_bar }
  ...
  in_thread { remix_fill_bar } if final_bar
  sleep 4
end
